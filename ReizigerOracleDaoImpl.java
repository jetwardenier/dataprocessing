...
public class ReizigerOracleDaoImpl extends OracleBaseDao implements ReizigerDao {
	...
	public List<Reiziger> findAllByGBdatum(String GBdatum) {
		...
		try {
			...
			String strQuery = "SELECT * FROM REIZIGER WHERE gebortedatum = to_date(" + "'" + GBdatum + "'"
					+ ", 'DD-MM-YYYY')";
	...
	public Reiziger save(Reiziger reiziger) {
		try {
			...
			insertStmt.executeQuery("INSERT INTO reiziger(reizigerid, voorletters, achternaam)	VALUES('"
					+ reizigeridint + "','" + voorletter + "','" + achternaam + "')");
	...
	public Reiziger update(Reiziger reiziger) {
		try {
			...
			String q = "Update REIZIGER SET voorletters = '" + voorletter + "', ACHTERNAAM = '" + achternaam
	...
	public boolean delete(Reiziger reiziger) {
		try {
			...
			ResultSet myRs = myStmt
					.executeQuery("delete from REIZIGER " + "where REIZIGERID = " + reiziger.getReizigerId());
			return true;
		} catch (Exception exc) {
			exc.printStackTrace();
			return false;
